...
def upload_file():
   if request.method == 'POST':
      ...
      if(thereIsSpeech):
        ...
        #outPathForDownSample = pathForDownSample
        outPathForDownSample = os.getcwd() + "/"+outputName.split(".wav")[0] +"_down.wav"
        tempCopy = os.getcwd() + "/audio_output/"+outputName.split(".wav")[0] +"_down.wav"
        downsample.downsampleWav(pathForDownSample, outPathForDownSample)
        copyfile(outPathForDownSample,tempCopy)
        ...
        speech = python_speech2txt.speech2text("audio_down","decoding/result/decoded_output-1.match")
        return speech
