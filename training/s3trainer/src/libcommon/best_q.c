/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/libcommon/RCS/best_q.c,v 1.1 97/07/16 11:36:22 eht Exp $
 *
...
static char rcsid[] = "@(#)$Id: best_q.c,v 1.1 97/07/16 11:36:22 eht Exp $";
...
/* ADDITION FOR CONTINUOUS_TREES; 20 May 98 */
    ...
    yes_dnom = (float64*) ckd_calloc(n_stream, sizeof(float64));
    no_dnom = (float64*) ckd_calloc(n_stream, sizeof(float64));
...
/* END MODIFICATION FOR CONTINUOUS_TREES */
	...
	for (s = 0, einc = 0; s < n_state; s++) {
	    for (j = 0; j < n_stream; j++) {
	        for (k = 0, yes_dnom[j] = 0; k < n_density; k++) {
		    yes_dnom[j] += yes_dist[s][j][k];
	        }
	        if (yes_dnom[j] == 0) 
		    break;
	    }
	    if (j != n_stream || yes_dnom[j] == 0)
		break; /* Counts are 0 for some stream */

	
	    for (j = 0; j < n_stream; j++) {
	        yes_norm = 1.0 / yes_dnom[j];
		for (k = 0; k < n_density; k++) {
	    ...
	    for (j = 0; j < n_stream; j++) {
	        for (k = 0, no_dnom[j] = 0; k < n_density; k++) {
		    no_dnom[j] += no_dist[s][j][k];
	        }
	        if (no_dnom[j] == 0) 
		    break;
	    }
	    if (j != n_stream || no_dnom[j] == 0)
		break; /* Counts are 0 for some stream */
	
	    for (j = 0; j < n_stream; j++) {
	        no_norm = 1.0 / no_dnom[j];
		for (k = 0; k < n_density; k++) {
   ...
   continuous */
	        ...
	        for (j = 0; j < n_stream; j++) {
   		        ...
   		        for (k = 0; k < veclen[j]; k++) {
                    ...
                    y_ent +=  yes_dnom[j] * ent_cont(yes_means[s][j],yes_vars[s][j],veclen[j]);
                    n_ent +=  no_dnom[j] * ent_cont(no_means[s][j],no_vars[s][j],veclen[j]);
                }
...
/* END ADDITION FOR CONTINUOUS_TREES */
	...
	*out_best_q = NULL;
    ...
    ckd_free((void *)no_id);
    ckd_free((void *)yes_dnom);
    ckd_free((void *)no_dnom);
