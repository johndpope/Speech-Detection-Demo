/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/libcommon/RCS/dtree.c,v 1.6 97/07/23 10:49:21 eht Exp Locker: eht $
 *
...
static char rcsid[] = "@(#)$Id: dtree.c,v 1.6 97/07/23 10:49:21 eht Exp Locker: eht $";
...
{
    if (node == NULL){
	E_WARN("Node is empty. Will print a dummy line in output file\n");
	fprintf(fp, "n_node 1\n0 - - 0 0\n");
	return;
    }
    fprintf(fp, "n_node %u\n", cnt_node(node));
   ...
   IN THE CASE OF CONTINUOUS HMMs (RATHER THAN MIXWs) */
	...
	for (j = 0, s_wt_ent = 0; j < n_stream; j++) {
	    norm = 1.0 / dnom[j];
		    ...
		    /* shouldn't be necessary. Actually this is a bad thing to do 
		    if (dist[j][k] < mwfloor)
		        dist[j][k] = mwfloor;
		    */
	        }
...
/* END ADDITION FOR CONTINUOUS_TREES */
	...
	for (j = 0; j < n_stream; j++) {
	    ...
	    for (k = 0; k < n_density; k++) {
		dist[s][j][k] *= norm;
		/* Dont floor here; flooring shouldve been done on mixw_occs
