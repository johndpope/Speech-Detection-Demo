/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/bldtree/RCS/main.c,v 1.1 97/07/16 11:36:22 eht Exp Locker: eht $
 *
...
/* END ADDITION FOR CONTINUOUS_TREES */
    ...
    *out_mdef = mdef;
    ...
    if (acmod_set_has_attrib(mdef->acmod_set, targ_base, "filler")) {
	E_ERROR("%s is a filler phone! No decision tree will be built\n",phn);
...
/* ADDITIONS FOR CONTINUOUS_TREES 18 May 98.
   ...
   /* DELETED CHECK. SHOULD NOT BE NEEDED */
/*
...
/* MODIFICATIONS FOR CONTINUOUS_TREES: Added "if (continuous==0)" to lines relating
   to wt_ent */
	...
	for (i = 0; i < n_model; i++) {
		...
		/* Floor the counts, not the mixws */
		for (k = 0; k < n_density; k++) {
		    if (mixw_occ[i][s][j][k] < mwfloor*dnom)
		    mixw_occ[i][s][j][k] = mwfloor*dnom;
		}
		for (k = 0, dnom = 0; k < n_density; k++) {
		    dnom += mixw_occ[i][s][j][k];
		}
...
/* END MODIFICATION FOR CONTINUOUS_TREES */
    ...
    if (n_model == 0) {
	/* No triphones here. Return a dummy tree */
	E_WARN("Writing out a dummy tree with no branches\n");
        fp = fopen(cmd_ln_access("-treefn"), "w");
        if (fp == NULL) {
	    E_FATAL_SYSTEM("Unable to open %s for writing",
		       cmd_ln_access("-treefn"));
        }
        print_final_tree(fp, NULL, pset);
        fclose(fp);
	return 1;  /* Quit */
    }

fprintf(stderr,"N model = %d\n",n_model); fflush(stdout);
