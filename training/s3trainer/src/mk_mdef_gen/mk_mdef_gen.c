			...
			phnhashelement_t ***phnhash)
{
	...
	for (i=1; i<=nwords; i++){/* Indices account for padded wordarr array */
		...
		/* Check for bogus triphone SIL x SIL; fillerphn = SIL
		   May occur for words with prons with both filler phones
		   and regular phones */
		if (tphnptr == NULL) continue; 
		(tphnptr->count)++;
		nswdtphns++;
	    }
	    else {
		...
		phnptr->count++;
		    ...
		    tphnptr = lookup(basephone,lctxt,rctxt,wordposn,tphnhash);
		    /* Check for bogus triphone SIL x SIL; fillerphn = SIL
		       May occur for words with prons which combine filler 
		       phones and regular phones */
		    if (tphnptr == NULL) continue; 
		    (tphnptr->count)++; 
                    nbwdtphns++;
	        }
		...
		tphnptr = lookup(basephone,lctxt,rctxt,wordposn,tphnhash);
		/* Check for bogus triphone SIL x SIL; fillerphn = SIL
		   May occur for words with prons which combine filler phones
		   and regular phones */
		if (tphnptr == NULL) continue; 
		(tphnptr->count)++;
		newdtphns++;
...
int32 find_threshold(hashelement_t  **triphonehash)
{
	...
	while (triphone_el != NULL){
	    ...
	    if (cnt > 0) {
	        tottph += cnt;
	        countofcounts[(cnt<CEILING ? cnt : CEILING)]++; unique++;
	        if (cnt >= mincnt) {
	            ltottph ++;
