/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/kmeans_init/RCS/main.c,v 1.1 97/07/16 11:36:22 eht Exp Locker: eht $
 *
...
setup_obs_multiclass(uint32 ts, uint32 strm, uint32 n_frame, const uint32 *veclen)
{
    ...
    vector_t **feat;
    int ret;
    uint32 ignore = 0;	/* keep programs like purify from spewing uninit mem errs */
    uint32 d_ts;
    uint32 n_sv_frame;
	  ...
	  uint32 vartiethr)
{
    ...
    vector_t  *mean_acc;
    vector_t  *var_acc;
    float64 ol, *p_ol, recip_ol, ttt, diff, log_tot_ol = 0, p_log_tot_ol = 0;
    gauden_t *g;
    float64 **norm, *den;
    float64 a_den, log_a_den;
    float32 mixw_norm, cb_norm;
...
{
    uint32 blksz;
    uint32 n_frame_cache;
    vector_t ***mean;
    vector_t ***var;
    float32  ***mixw;
    char fn[MAXPATHLEN+1];
    uint32 n_frame;
    uint32 rd_chksum, chksum, ignore = 0;
    codew_t *label;
    uint32 n_corpus = 0;
    float64 sqerr;
    float64 tot_sqerr;
    segdmp_type_t t;
    uint32 s, i, j, ii, ts, n;
    timing_t *km_timer;
    timing_t *var_timer;
    timing_t *em_timer;
