...

uint32
v3_feat_blksize(uint32 veclen)
{
    v3_feat_set_in_veclen(veclen);
    return vecsize[0];
}

const uint32 *
v3_feat_vecsize(uint32 veclen)
{
    v3_feat_set_in_veclen(veclen);
    return vecsize;
}
	        ...
	        uint32 veclen)
{
    ...
    uint32 n_frame;
    uint32 i, j;
    void v3_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen);
    
    v3_feat_set_in_veclen(veclen);
    n_frame = svd_n_frame = *inout_n_frame;

    out = v3_feat_alloc(n_frame,veclen);
...

void
v3_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen)
{
    uint32 i, j, k;
