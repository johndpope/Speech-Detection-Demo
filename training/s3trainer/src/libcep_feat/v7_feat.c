...

uint32
v7_feat_blksize(uint32 veclen)
{
    v7_feat_set_in_veclen(veclen);
    return vecsize[0];
}

const uint32 *
v7_feat_vecsize(uint32 veclen)
{
    v7_feat_set_in_veclen(veclen);
    return vecsize;
}
		...
		uint32 veclen)
{
    ...
    uint32 dd_begin;
    uint32 dd_end;
    void v7_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen);
    ...
    out = v7_feat_alloc(n_frame, veclen);
...

void
v7_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen)
{
    uint32 i, j, k;
