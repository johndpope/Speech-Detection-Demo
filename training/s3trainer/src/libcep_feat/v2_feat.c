/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/libcep_feat/RCS/v2_feat.c,v 1.4 97/07/16 11:36:22 eht Exp $
 *
...
static char rcsid[] = "@(#)$Id: v2_feat.c,v 1.4 97/07/16 11:36:22 eht Exp $";
...

uint32
v2_feat_blksize(uint32 veclen)
{
    v2_feat_set_in_veclen(veclen);
    return vecsize[0];
}

const uint32 *
v2_feat_vecsize(uint32 veclen)
{
    v2_feat_set_in_veclen(veclen);
    return vecsize;
}
		...
		uint32 veclen)
{
    ...
    uint32 dd_begin;
    uint32 dd_end;
    void v2_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen);
    ...
    out = v2_feat_alloc(n_frame,veclen);
...

void
v2_mfcc_print(vector_t *mfcc, uint32 n_frame, uint32 veclen)
{
    uint32 i, j, k;
