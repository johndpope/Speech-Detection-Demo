...
char *agc_type;
char *normvar;
uint32 *feat_vectorsize;
uint32 feat_blocksize;
...
feat_set_in_veclen(uint32 len)
{
    ...
    feat_vectorsize = (uint32 *)ckd_calloc(feat_conf[fid].n_stream(),sizeof(uint32));
    f_vecsize = feat_conf[fid].vecsize(mfcc_len);
    for (i=0; i<feat_conf[fid].n_stream(); i++)
	feat_vectorsize[i] = f_vecsize[i];
    feat_blocksize = feat_conf[fid].blksize(mfcc_len);
}

uint32
feat_mfcc_len()
	     ...
	     uint32 *inout_n_frame)
{
    ...
    cmn_type = (char*) cmd_ln_access("-cmn");
    agc_type = (char*) cmd_ln_access("-agc");
    normvar = (char*) cmd_ln_access("-varnorm");
