/*********************************************************************
 *
 * $Header: /net/alpha4/usr1/bhiksha/work/sphinx-3/sphinx_iii/src/libio/RCS/corpus.c,v 1.3 1997/07/18 14:04:51 eht Exp $
 *
 ...
 *********************************************************************/

static char rcsid[] = "@(#)$Id: corpus.c,v 1.3 1997/07/18 14:04:51 eht Exp $";
...
/* ADDITION FOR PARALLEL BW */
static char *
mk_filename2(uint32 type, char *rel_path);
/* END ADDITION*/
...
/* ADDITION FOR PARALLEL BW */
static const char *data2_dir[N_DATA_TYPE];
/* END ADDITION */
...
/* ADDITION FOR PARALLEL BW */
static const char *extension2[N_DATA_TYPE];
/* END ADDITION */
...
/* ADDITION FOR PARALLEL BW */
static int is_flat2[N_DATA_TYPE];
/* END ADDITION */
...
/* ADDITION FOR PARALLEL BW */
static int32 requires_mfcc2 = FALSE;
/* END ADDITION */
...

int
corpus_set_mfcc2_dir(const char *dir)
{
    requires_mfcc2 = TRUE;

    data2_dir[DATA_TYPE_MFCC] = dir;
    is_flat2[DATA_TYPE_MFCC] = FALSE;
...

int
corpus_set_mfcc2_ext(const char *ext)
{
    extension2[DATA_TYPE_MFCC] = ext;
...
corpus_init()
{
   ...
   /* ADDITION FOR PARALLEL BW */
    if (requires_mfcc2 &&
	extension2[DATA_TYPE_MFCC] == NULL) {
	E_ERROR("No MFCC extension given\n");
		...
		uint32 *veclen)
{
    ...
    do {
	if ((cur_ctl_sf == NO_FRAME) && (cur_ctl_ef == NO_FRAME)) {
	    ret = areadfloat(mk_filename2(DATA_TYPE_MFCC, cur_ctl_path),
			     &coeff, (int *)&n_c);
	}
	else if ((cur_ctl_sf != NO_FRAME) && (cur_ctl_ef != NO_FRAME)) {
	    ret = areadfloat_part(mk_filename2(DATA_TYPE_MFCC, cur_ctl_path),
				  cur_ctl_sf * S2_CEP_VECLEN,
				  (cur_ctl_ef + 1) * S2_CEP_VECLEN - 1,
				  &coeff, (int *)&n_c);
	}
    ...
    if ((ret == 0) && (cur_ctl_sf != NO_FRAME) && (cur_ctl_ef != NO_FRAME)) {
	E_ERROR("Region [%d %d] for %s extends beyond end of file for feature2\n",
		cur_ctl_sf, cur_ctl_ef, corpus_utt());
    }
...
/* ADDED BY TO READ FEATURES OF GENERIC LENGTH. TO ALLOW
...
/* ADDITION FOR PARALLEL BW */
int
corpus_get_generic_featurevec2(vector_t **mfc,
			      uint32 *n_frame,
			      uint32 veclen)
{
    vector_t *out;
