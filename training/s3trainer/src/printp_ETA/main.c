/*********************************************************************
 *
 * $Header: /net/alf19/usr2/eht/s3/src/printp/RCS/main.c,v 1.12 97/07/16 11:17:32 eht Exp Locker: eht $
 *
...
print_tmat(const char *fn)
{
    ...
    float32 sum;
    float32 eta, scalingfactor, prior;
    int32 numincomingedges, skipstate;
    
    E_INFO("Reading %s%s\n",
	   (normalize ? " and normalizing." : "."),
	   fn);
    
    eta = *(float32 *)cmd_ln_access("-tmateta");
    skipstate = *(int32 *)cmd_ln_access("-skipstate");
    if (skipstate) 
        numincomingedges = 3;
    else
        numincomingedges = 2;
    prior = 1/(float)numincomingedges;
    scalingfactor = pow(prior,1.0/eta);
	...
	for (j = 0; j < n_state_pm-1; j++) {
	    ...
	    for (k = 0; k < n_state_pm; k++) {
	        printf(e_fmt_ld_sp, scalingfactor*tmat[i][j][k]/sum);
	    }
	    printf("\n");
	}
    }
