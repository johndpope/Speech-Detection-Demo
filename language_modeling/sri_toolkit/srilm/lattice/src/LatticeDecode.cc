...
#ifndef lint
static char Copyright[] = "Copyright (c) 2004-2008 SRI International.  All Rights Reserved.";
static char RcsId[] = "@(#)$Header: /home/srilm/devel/lattice/src/RCS/LatticeDecode.cc,v 1.7 2008/07/07 22:39:37 stolcke Exp $";
#endif
...
public:
  ...
  int	m_NumPaths; // number of paths
...
INSTANTIATE_CACHEDMEM(PathLink);
INSTANTIATE_CACHEDMEM(LatticeDecodePath);
INSTANTIATE_CACHEDMEM(NodePathInfo);
INSTANTIATE_CACHEDMEM(NBestPath);
...
static void
freePaths(NodePathInfo ** nodeinfo, int finalPosition)
{

  for (int i = 0; i <= finalPosition; i++) {
    NodePathInfo & info = *nodeinfo[i];
     for (int j = 0; j < info.m_NumPaths; j++)
...
NodePathInfo **
Lattice::decode(unsigned contextLen, LM * lm, int finalPosition, NodeIndex * sortedNodes, 
		double beamwidth, float lmscale, int nbest, int maxFanIn, LogP logP_floor)
{

  VocabIndex context[32];
  int i;  
		     ...
		     LM * lm , unsigned contextLen, double beamwidth, LogP logP_floor)
{
  ...
  LogP result = decode1Best(winfo, maxWords, ignoreWords, lm, contextLen, beamwidth, logP_floor);
		     ...
		     LM * lm, unsigned contextLen, double beamwidth, LogP logP_floor)
{
  ...
  NodePathInfo ** nodeinfo = decode(contextLen, lm, finalPosition, sortedNodes,
				    beamwidth, lmscale, 0, 0, logP_floor);
...
Lattice::decodeNBest(unsigned N, NBestOptions &nbestOut, SubVocab &ignoreWords, 	     
		     ...
		     const char *multiwordSeparator, LogP logP_floor)
{
  if (beamwidth == 0) 
    beamwidth = 1e30;
  ...
  while(outputHyps < N && !pathQueue.empty()) {
    ...
    if (topPath->m_LatPath->m_Prev == 0) {
      ...
      int n = topPath->m_LatPath->m_NumPreds;
