...
#ifndef lint
static char Copyright[] = "Copyright (c) 2000-2008 SRI International.  All Rights Reserved.";
static char RcsId[] = "@(#)$Id: nbest-optimize.cc,v 1.57 2008/05/27 16:47:56 stolcke Exp $";
#endif
 ...
 */
void
printTopNbestHyps(RefString id, NBestList &nbest, int num, char * dirname)
{
    unsigned numHyps = nbest.numHyps();
							...
							unsigned &numWords)
{
	...
	int pos, nl, nr, i, rl;
	if (sscanf(line, "%d%d%n", &nl, &nr, &pos) != 2) {
...
main(int argc, char **argv)
{
	...
	*nbestScores.insert(id) = scores;
    ...
    if (optimizeBleu) {
        ...
        if (useAvgRefLeng || numReferences == 1) 
	  bleuRefLength = numRefWords;
	else if (useMinRefLeng) {

	    bleuRefLength = refLength[0];
	    for (unsigned i = 1; i < numReferences; i++) {
	        if (bleuRefLength > refLength[i])
		  bleuRefLength = refLength[i];
	    }	    	      
	}
    }
